...
import {
    Users, Search, Plus, Edit2, Trash2, Key, CheckCircle, XCircle, Mail
} from 'lucide-react'
...
const CompanyUsers = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const companyUsers = usersRes.data.filter(u => u.tenant_id === currentUser?.tenant_id)
            setUsers(companyUsers)
        } catch (error) {
            console.error('Error fetching data:', error)
            // Fallback data for demo purposes if API fails or is empty
            if (process.env.NODE_ENV === 'development') {
                // Keep existing users if any, or maybe don't overwrite with empty if error
            }
        } finally {
            setLoading(false)
        }
    ...
    const handleCreateUser = async (e) => {
        ...
        try {
            const payload = {
                ...formData,
                tenantId: currentUser.tenant_id // Force current tenant
            }
    ...
    const handleUpdateUser = async (e) => {
        ...
        try {
            const payload = {
                ...formData,
                tenantId: currentUser.tenant_id // Ensure tenant doesn't change
            }
        ...
        <div className="p-6 max-w-7xl mx-auto">
            <div className="mb-8 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-600 mt-2">Administrer ansatte i {currentUser?.companyName || 'selskapet'}</p>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="text-left py-3 px-4 text-sm font-medium text-gray-700">Ansatt</th>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-xl max-w-lg w-full p-6">
                        <h2 className="text-xl font-bold mb-6">
                            {editingUser ? 'Rediger ansatt' : 'Ny ansatt'}
                        </h2>
                        ...
                        <form onSubmit={editingUser ? handleUpdateUser : handleCreateUser}>
                            ...
                            <div className="flex justify-end gap-3 mt-6">
                                ...
                                >
                                    {editingUser ? 'Lagre endringer' : 'Opprett ansatt'}
                                </button>
                            </div>
                        </form>
                    </div>
